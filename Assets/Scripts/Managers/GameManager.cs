...
public class GameManager : SingletonMB<GameManager>
{
    ...
    private const string c_DisablePowerUpsCheat = "DISABLE_POWER_UP";
    ...
    void Update()
    {
        ...
        var canSpawPowerups = !CheatManager.GetCheatStatus(c_DisablePowerUpsCheat);

        if (Time.time - m_LastPowerUpTime > m_PowerUpRate && canSpawPowerups)
        {
            m_PowerUpRate = Random.Range(c_MinPowerUpRate, c_MaxPowerUpRate);
            m_LastPowerUpTime = Time.time;
        ...
        if (Time.time - m_LastBrushTime > c_BrushRate && canSpawPowerups)
        {
            m_LastBrushTime = Time.time;
